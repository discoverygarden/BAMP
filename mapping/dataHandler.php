...
switch($_GET['type']){
  ...
  case 'filterFields':
      ...
      $query.= "WHERE table_name IN ('bamp_wild_sampling_instances','bamp_wild_fish_samples') ";
      //Fields to exclude as filters
      $query.= "AND column_name NOT IN ('id','changelog','trip_id','year_rep_route_site','comments','site_number','fish','route','tissue_damage', 'latitude','longitude', 'date', 'waypoint')";
      $result = mysql_query($query, $dbh) or die(mysql_error($dbh));
      ...
      while($row = mysql_fetch_assoc($result)){
        switch($row['table_name']){
          case 'bamp_wild_sampling_instances':
            $tableName = 'si';
            $tableNameStr = 'Sampling_Instances';
          break;
          case 'bamp_wild_fish_samples':
            $tableName = 'fs';
            $tableNameStr = 'Fish_Samples';
          break;
        }//end switch
